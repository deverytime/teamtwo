...
<header class="bg-white border-b border-slate-200 relative z-50 h-[64px]">
	<div class="navbar max-w-6xl w-full mx-auto px-4 py-0 h-full min-h-0">
		...
		<div class="flex-none h-full">
			<ul class="flex h-full text-slate-700 font-semibold items-center">
				...
				<li class="h-full">
					<a href="/teamtwo/plan/list.do" class="relative h-full flex items-center px-4 hover:bg-slate-50 transition-colors ${uri.contains('/plan/') ? 'nav-active' : ''}">학습계획</a>
				</li>
				<li class="h-full">
					<a href="/teamtwo/study/study-list.do" class="relative h-full flex items-center px-4 hover:bg-slate-50 transition-colors ${uri.contains('/study/') ? 'nav-active' : ''}">스터디</a>
				</li>
				...
				<c:choose>
					<c:when test="${empty auth}">
						<li class="h-full"><a href="/teamtwo/user/login.do" class="relative h-full flex items-center px-4 hover:bg-slate-50 transition-colors ${uri.contains('login') || uri.contains('register') || uri.contains('find') ? 'nav-active' : ''}">로그인/회원가입</a></li>
					</c:when>
					<c:otherwise>
						<li class="h-full">
							<a href="/teamtwo/user/mypage.do" 
						...
						<li class="h-full"><a href="/teamtwo/user/logout.do" class="relative h-full flex items-center px-4 hover:bg-slate-50 transition-colors">로그아웃</a></li>
						
						<c:if test="${authDto.type == 1}">
							<li class="h-full"><a href="/teamtwo/admin/admin.do" class="relative h-full flex items-center px-4 text-point-500 hover:bg-slate-50 transition-colors ${uri.contains('/admin/') ? 'nav-active' : ''}">관리자 메뉴</a></li>
						</c:if>
					</c:otherwise>
				</c:choose>
			</ul>
